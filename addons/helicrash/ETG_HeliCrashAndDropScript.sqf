...
_minDist = 100; 											//min distance from center of map in meters
_maxDist = 2500; 											//max distance from center of map in meters



//Change this path,if you have moved the script in a special folder of your mission, its the one to the script itself
_ETG_ScriptPath = "addons\helicrash\ETG_HeliCrashAndDropScript.sqf";
...
//stratis - right left bottom and right left top positions

_ETG_StartionPositions = [[8073,0,0],[127,0,0],[8029,-0,0],[174,-0,0]];
...
_CenterOfMap = []; //if blank defaults to worlds static safe center
...
////////////////////////////////////////////////////////////////
	...
	//Get the position of the drop/crash	
		_ETG_Main_Coordinates = [_CenterOfMap, _minDist, _maxDist, 10, 0, 200, 0] call BIS_fnc_findSafePos;
...
//if its a cargo drop do this		
	if (_ETG_Mission_Type == "Cargo_Drop") then [ {
		...
		//waits until the distance to the drop point is > 500
			waitUntil {_ETG_Transport_Vehicle distance _ETG_Main_Coordinates < 600};	
		...
		//Delete the heli
		//Stratis is much smaller than altis. The altis value is set to 10000, 
		//which can cause the helis to never delete on stratis. 
			waituntil {_ETG_Transport_Vehicle distance [0,0,0] < 1500};
...
//Run if its a heli crash
	if (_ETG_Mission_Type == "Heli_Crash") then [ {
		...
		//Get two more positions
			_ETG_2cnd_Coordinates = [_CenterOfMap, _minDist, _maxDist, 10, 0, 200, 0] call BIS_fnc_findSafePos;
			_ETG_3rd_Coordinates = [_CenterOfMap, _minDist, _maxDist, 10, 0, 200, 0] call BIS_fnc_findSafePos;
